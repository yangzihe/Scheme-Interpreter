...
class Frame(object):
    ...
    def make_call_frame(self, formals, vals):
        ...
        if len(vals) == len(formals):
            while vals and formals:
                frame.define(formals.first, vals.first)
                vals, formals = vals.second, formals.second
            return frame
        else:
            raise SchemeError("number of values and args don't match!")
...
def do_lambda_form(vals, env):
    ...
    "*** YOUR CODE HERE ***"
    begin_body = vals.second[0]
    for each in vals.first:
        if each == nil or type(each) != str:
            raise SchemeError("variable is not type string")
    if len(vals.second) > 1:
        begin_body = Pair('begin', vals.second)
    return LambdaProcedure(formals, begin_body, env)
...
def do_mu_form(vals):
    ...
    if len(vals.second) > 1:
        begin_body = Pair('begin', vals.second)
    return MuProcedure(formals, begin_body)
...
def do_define_form(vals, env):
    ...
    if scheme_symbolp(target):
        ...
        "*** YOUR CODE HERE ***"
        if type(target.first) == int:
            raise SchemeError("variable cannot be int")
        target_exp = Pair(target.second, vals.second)
        target_exp = do_lambda_form(target_exp, env)
        env.define(target.first, target_exp)
        return target.first
    else:
...
def do_let_form(vals, env):
    ...
    for pair in bindings:
        names, values = Pair(pair[0], names), Pair(scheme_eval(pair[1], env), values)
...
def do_or_form(vals, env):
    ...
    "*** YOUR CODE HERE ***"
    for elem in range(len(vals)):
        if scheme_true(scheme_eval(vals[elem], env)):
            return vals[elem]
    return False
...
def do_begin_form(vals, env):
    ...
    for elem in range(len(vals)-1):
        value = scheme_eval(vals[elem], env)
